...
import { Plus, MapPin, Search, Package, Cat, User, Car, ImageOff, ArrowRight } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {user ? (
          <Link
            to="/ftl/new"
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-violet-600 text-white rounded-lg text-sm font-medium hover:bg-violet-700 transition-smooth"
          >
            <Plus size={18} />
            {t('ftl.newAlert')}
          </Link>
          ...
          <Link
            to="/login"
            className="inline-flex items-center gap-2 px-5 py-2.5 border border-violet-200 dark:border-violet-800 text-violet-700 dark:text-violet-300 rounded-lg text-sm font-medium hover:bg-violet-50 dark:hover:bg-violet-900/20 transition-smooth"
          >
            {t('ftl.signInToPost')}
          </Link>
        )}
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 flex gap-4 animate-pulse"
            >
        ...
        <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
          ...
          {user && (
            <Link
              to="/ftl/new"
              className="inline-flex items-center gap-1 text-sm text-violet-600 dark:text-violet-400 font-medium hover:underline mt-2"
            >
              {t('ftl.newAlert')}
              <ArrowRight size={14} />
            </Link>
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-1">
                  ...
                  <div className="flex items-center gap-3 mt-1 text-xs text-slate-500 dark:text-slate-400 flex-wrap">
                    ...
                    {(alert as any).profiles?.full_name && (
                      <>
                        <span>·</span>
                        <span>
                          {t('ftl.postedBy')} {(alert as any).profiles.full_name}
                        </span>
                      </>
                    )}
                  </div>
